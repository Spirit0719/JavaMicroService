...
//    }
    ...
    public ApiRequest filter(OperatorType operator, String field, Object... values) {
        return this.filter(operator, field, null, values);
    }

    public ApiRequest filter(OperatorType operator, String field, FieldDescModel descModel, Object... values) {
        if (filterList == null) {
            filterList = new ArrayList<>();
        }
        try {
            ...
            if (OperatorType.isUnary(operator)) {
                ...
                filterList.add(new ApiRequestFilter(operator, field, descModel, values[0]));
            } else if (OperatorType.isBinary(operator)) {
                if (values.length != 2) {
                    throw new IllegalArgumentException("双目运算符参数个数不为1个");
                }
                filterList.add(new ApiRequestFilter(operator, field, descModel, Lists.newArrayList(values)));
    ...
    public ApiRequest filterEqual(String field, FieldDescModel descModel, Object value) {
        return this.filter(OperatorType.EQ, field, descModel, value);
    }
    ...
    public ApiRequest filterIsNull(String field) {
        return this.filter(OperatorType.ISNULL, field, "");
    }
    ...
    public ApiRequest filterLike(String field, FieldDescModel descModel, Object value) {
        return this.filter(OperatorType.LIKE, field, descModel, value);
    }
